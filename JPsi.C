  ...
  Double_t A_2g = 6.499e3*v*pow(1-x,2)*pow((s-Mp*Mp),2)/(Mj*Mj);
  Double_t A_3g = 2.894e3*v*pow(1-x,0)*pow((s-Mp*Mp),2)/(Mj*Mj*Mj*Mj);
 
  // Below are fits to Cornell data (without Brodsky's 2 or 3 gluon exchange model)
...
if (particle.Vect().Mag()>pmin && particle.Vect().Mag() < pmax && TMath::Abs(horzA)<hAcc && TMath::Abs(vertA)<vAcc)
{
  ...
  void JPsi(Int_t n, Int_t seed, Double_t HMS_th, Double_t HMS_centp, Double_t SHMS_th, Double_t SHMS_centp) {
  ...
  for (Int_t i=0;i<n;i++) {
    ...
    j.SetXYZM(Ppp_cm*(-1)*(x),Ppp_cm*(-1)*(y),Ppp_cm*(-1)*(z),Mj2);
    ...
    //Bool_t HMS=specAcc(4.2,21.0*3.14159/180.0,24e-3,70e-3,0.10,0.10,electron);

    //Bool_t SHMS=specAcc(5.8,-15.0*3.14159/180.0,20e-3,50e-3,0.15,0.25,positron);

...
#include "jpacPhoto_pentaquark.hh"
#include <TF1.h>
#include <TMath.h>
#include <algorithm>
...
// =============================================================================
// Constructor for lA::jpacPhoto_pentaquark
// =============================================================================
jpacPhoto_pentaquark::jpacPhoto_pentaquark(const configuration& cf,
    ...
    , decay_lminus_{static_cast<pdg_id>(
          abs(cf.get<int>(path / "vm_decay_lepton_type", 11)))}
    , spin_parity_{cf.get_vector<int>(path / "spin_parity")}
    , mass_{cf.get_vector<double>(path / "mass")}
    , width_{cf.get_vector<double>(path / "width")}
    , name_{cf.get_vector<std::string>(path / "name")}
    , photo_balance_{cf.get<double>(path / "photo_balance")}
    , branching_{cf.get<double>(path / "branching")}
    , dipole_n_{cf.get<double>(path / "dipole_n")}
    , reaction_{init_reaction()}
    , components_{init_components()}
    , ampl_{init_ampl()}
    , max_t_range_{calc_max_t_range(cf)}
    , max_{calc_max_xsec(cf)} {
  LOG_INFO("jpacPhoto_pentaquark", "t range [GeV^2]: [" +
                                       std::to_string(max_t_range_.min) + ", " +
                                       std::to_string(max_t_range_.max) + "]");
  // at this stage, if "components_" initialize without issue we know we have
  // a valid configuration, so we can safely access the elements from all
  // vectors
  for (int i = 0; i < name_.size(); ++i) {
    LOG_INFO("jpacPhoto_pentaquark",
             "Resonant component " + std::to_string(i) + ": " + name_[i] +
                 ", with mass " + std::to_string(mass_[i]) + "GeV, width " +
                 std::to_string(width_[i]) + ", spin/parity " +
                 std::to_string(abs(spin_parity_[i])) +
                 (spin_parity_[i] < 0 ? "/2-" : "/2+"));
  }
  LOG_INFO("jpacPhoto_pentaquark",
           "photocoupling balance: " + std::to_string(photo_balance_));
  LOG_INFO("jpacPhoto_pentaquark",
           "branching fraction to J/psi-p: " + std::to_string(branching_));
  LOG_INFO("jpacPhoto_pentaquark",
           "'Dipole' FF power: " + std::to_string(dipole_n_));
  LOG_INFO("jpacPhoto_pentaquark", "VM: " + std::string(vm_.pdg()->GetName()));
  LOG_INFO("jpacPhoto_pentaquark",
           "recoil: " + std::string(recoil_.pdg()->GetName()));
}
std::unique_ptr<jpacPhoto::reaction_kinematics>
jpacPhoto_pentaquark::init_reaction() const {
  auto reaction = std::make_unique<jpacPhoto::reaction_kinematics>(vm_.mass());
  reaction->set_JP(1, -1);
  return reaction;
}
std::vector<std::unique_ptr<jpacPhoto::baryon_resonance>>
jpacPhoto_pentaquark::init_components() const {
  tassert(spin_parity_.size() > 0 && spin_parity_.size() == mass_.size() &&
              mass_.size() == name_.size() && mass_.size() == width_.size(),
          "Invalid or incompatible array definition of pentaquark components");
  std::vector<std::unique_ptr<jpacPhoto::baryon_resonance>> comps;
  for (int i = 0; i < name_.size(); ++i) {
    auto comp = std::make_unique<jpacPhoto::baryon_resonance>(
        reaction_.get(), abs(spin_parity_[i]), (spin_parity_[i] < 0 ? -1 : 1),
        mass_[i], width_[i], name_[i]);
    comp->set_params({branching_, photo_balance_});
    comps.push_back(std::move(comp));
  }
  return comps;
}
jpacPhoto::amplitude_sum jpacPhoto_pentaquark::init_ampl() {
  std::vector<jpacPhoto::amplitude*> ampls;
  for (const auto& comp : components_) {
    ampls.push_back(comp.get());
  }
  jpacPhoto::amplitude_sum ampl{reaction_.get(), ampls, "jpacPhoto_pentaquark"};
  return ampl;
}

lA_event jpacPhoto_pentaquark::generate(const lA_data& initial) {
  ...
  LOG_JUNK("jpacPhoto_pentaquark", "dipole: " + std::to_string(xs_dipole));
...
// =============================================================================
// jpacPhoto_pentaquark::calc_max_xsec(cf)
//
...
// =============================================================================
double jpacPhoto_pentaquark::calc_max_xsec(const configuration& cf) /*const */ {
  double max = -1;
  for (const double mass : mass_) {
    auto xsec = [&](double* tt, double*) {
      return dsigma_dt(mass * mass, tt[0]);
    };
    TF1 tf_xsec{"tf_xsec", xsec, max_t_range_.min, max_t_range_.max, 0};
...
// =============================================================================
// jpacPhoto_pentaquark::calc_max_t_range(cf)
//
// Utility function for the generator initialization
//
...
// =============================================================================
interval<double>
jpacPhoto_pentaquark::calc_max_t_range(const configuration& cf) const {
  // hack
...
// =============================================================================
// jpacPhoto_pentaquark::dsigma_dt()
// jpacPhoto_pentaquark::dipole()
//
// Utility functions to calculate the cross section components
// =============================================================================
double jpacPhoto_pentaquark::dsigma_dt(const double s,
                                       const double t) /*const */ {
  return ampl_.differential_xsection(s, t);
}
double jpacPhoto_pentaquark::dipole(const double Q2) const {
  return physics::dipole_ff_vm(Q2, vm_.mass(), dipole_n_);
}
// =============================================================================
// jpacPhoto_pentaquark::threshold2()
//
...
// widht.
// =============================================================================
double jpacPhoto_pentaquark::threshold2(const particle& vm,
...
// the lab-frame
// =============================================================================
lA_event jpacPhoto_pentaquark::make_event(const lA_data& initial,
  ...
  auto& d1 = decay_products.second;
  {
    ...
    LOG_JUNK2("jpacPhoto_pentaquark",
              "SDME Values: r00: " + std::to_string(r00) + ", r10: " +
                  std::to_string(r10) + ", r1-1: " + std::to_string(r1m1));

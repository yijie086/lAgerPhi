...
std::complex<double> fT1(const double q2, const double Egamma, const double t,
  ...
  return fact * jpsi_T(nu, T_0) * exp(1.13 * 0.5 * t);
}
...
double TT(const double t, const double q2, const double Egamma,
  ...
  return (
      std::pow(e, 6) *
      ((2 * std::pow(ImT1, 2) * (2 * std::pow(m, 2) + std::pow(Mll, 2)) *
        (4 * std::pow(M, 2) - t)) /
           std::pow(Mll, 4) +
       (2 * (2 * std::pow(m, 2) + std::pow(Mll, 2)) * std::pow(ReT1, 2) *
        (4 * std::pow(M, 2) - t)) /
           std::pow(Mll, 4) -
       (std::pow(ImT1, 2) * std::pow(std::pow(Mll, 2) - t, 2) *
        std::pow(-4 * std::pow(M, 2) + t, 2) *
        (-4 * std::pow(a, 2) - 4 * a * b - std::pow(b, 2) +
         16 * std::pow(E, 2) * std::pow(M, 2) - 8 * E * M * std::pow(Mll, 2) -
         16 * std::pow(M, 2) * std::pow(Mll, 2) + std::pow(Mll, 4) +
         8 * E * M * t + 2 * std::pow(Mll, 2) * t + std::pow(t, 2))) /
           (std::pow(Mll, 4) * std::pow(4 * E * M - std::pow(Mll, 2) + t, 4)) -
       (std::pow(ReT1, 2) * std::pow(std::pow(Mll, 2) - t, 2) *
        std::pow(-4 * std::pow(M, 2) + t, 2) *
        (-4 * std::pow(a, 2) - 4 * a * b - std::pow(b, 2) +
         16 * std::pow(E, 2) * std::pow(M, 2) - 8 * E * M * std::pow(Mll, 2) -
         16 * std::pow(M, 2) * std::pow(Mll, 2) + std::pow(Mll, 4) +
         8 * E * M * t + 2 * std::pow(Mll, 2) * t + std::pow(t, 2))) /
           (std::pow(Mll, 4) * std::pow(4 * E * M - std::pow(Mll, 2) + t, 4)) -
       (std::pow(ImT1, 2) * (4 * std::pow(M, 2) - t) *
        (2 * a * b + std::pow(b, 2) -
         (4 * E * M - std::pow(Mll, 2) + t) * (std::pow(Mll, 2) + t))) /
           (std::pow(Mll, 4) * (-4 * E * M + std::pow(Mll, 2) - t)) -
       (std::pow(ReT1, 2) * (4 * std::pow(M, 2) - t) *
        (2 * a * b + std::pow(b, 2) -
         (4 * E * M - std::pow(Mll, 2) + t) * (std::pow(Mll, 2) + t))) /
           (std::pow(Mll, 4) * (-4 * E * M + std::pow(Mll, 2) - t)) +
       (4 * GEt * M * ReT1 *
        (-(b * (4 * std::pow(m, 2) + std::pow(Mll, 2)) *
           (std::pow(Mll, 2) - t)) -
         a * (std::pow(Mll, 2) - t) *
             (4 * std::pow(m, 2) + std::pow(Mll, 2) - t) +
         2 * b * E * M * (4 * std::pow(m, 2) + std::pow(Mll, 2) + t))) /
           (L * std::pow(Mll, 2) * t) +
       (std::pow(ImT1, 2) * (4 * std::pow(M, 2) - t) * (std::pow(Mll, 2) - t) *
        (-64 * std::pow(E, 3) * std::pow(M, 3) +
         48 * std::pow(E, 2) * std::pow(M, 2) * std::pow(Mll, 2) +
         96 * E * std::pow(M, 3) * std::pow(Mll, 2) -
         12 * E * M * std::pow(Mll, 4) -
         24 * std::pow(M, 2) * std::pow(Mll, 4) + std::pow(Mll, 6) -
         48 * std::pow(E, 2) * std::pow(M, 2) * t +
         32 * E * std::pow(M, 3) * t +
         16 * std::pow(M, 2) * std::pow(Mll, 2) * t + 3 * std::pow(Mll, 4) * t -
         20 * E * M * std::pow(t, 2) + 8 * std::pow(M, 2) * std::pow(t, 2) -
         std::pow(Mll, 2) * std::pow(t, 2) - 3 * std::pow(t, 3) +
         4 * std::pow(a, 2) * (4 * E * M - std::pow(Mll, 2) + t) +
         4 * a * b *
             (4 * E * M - 4 * std::pow(M, 2) - std::pow(Mll, 2) + 2 * t) +
         std::pow(b, 2) *
             (4 * E * M - 8 * std::pow(M, 2) - std::pow(Mll, 2) + 3 * t))) /
           (std::pow(Mll, 4) * std::pow(-4 * E * M + std::pow(Mll, 2) - t, 3)) +
       (std::pow(ReT1, 2) * (4 * std::pow(M, 2) - t) * (std::pow(Mll, 2) - t) *
        (-64 * std::pow(E, 3) * std::pow(M, 3) +
         48 * std::pow(E, 2) * std::pow(M, 2) * std::pow(Mll, 2) +
         96 * E * std::pow(M, 3) * std::pow(Mll, 2) -
         12 * E * M * std::pow(Mll, 4) -
         ...
         std::pow(b, 2) *
             (4 * E * M - 8 * std::pow(M, 2) - std::pow(Mll, 2) + 3 * t))) /
           (std::pow(Mll, 4) * std::pow(-4 * E * M + std::pow(Mll, 2) - t, 3)) -
       (4 * std::pow(GEt, 2) * std::pow(M, 2) *
        (32 * std::pow(E, 2) * std::pow(m, 2) * std::pow(M, 2) -
         16 * E * std::pow(m, 2) * M * std::pow(Mll, 2) +
         16 * std::pow(E, 2) * std::pow(M, 2) * std::pow(Mll, 2) +
         2 * std::pow(m, 2) * std::pow(Mll, 4) - 8 * E * M * std::pow(Mll, 4) +
         std::pow(Mll, 6) +
         std::pow(b, 2) * (-2 * std::pow(m, 2) - 4 * E * M + std::pow(Mll, 2)) +
         16 * E * std::pow(m, 2) * M * t -
         32 * std::pow(m, 2) * std::pow(M, 2) * t -
         4 * std::pow(m, 2) * std::pow(Mll, 2) * t +
         8 * E * M * std::pow(Mll, 2) * t +
         16 * std::pow(M, 2) * std::pow(Mll, 2) * t - 2 * std::pow(Mll, 4) * t +
         10 * std::pow(m, 2) * std::pow(t, 2) -
         3 * std::pow(Mll, 2) * std::pow(t, 2) +
         2 * a * b * (-4 * std::pow(m, 2) - 4 * E * M + std::pow(Mll, 2) + t) +
         std::pow(a, 2) * (-8 * std::pow(m, 2) + 4 * t))) /
           (L * (4 * std::pow(M, 2) - t) * std::pow(t, 2)) -
       (std::pow(ImT1, 2) * (4 * std::pow(M, 2) - t) *
        (16 * std::pow(E, 2) * std::pow(M, 2) * std::pow(Mll, 2) -
         8 * E * M * std::pow(Mll, 4) - 28 * std::pow(M, 2) * std::pow(Mll, 4) +
         std::pow(Mll, 6) - 32 * std::pow(E, 2) * std::pow(M, 2) * t +
         24 * E * M * std::pow(Mll, 2) * t +
         8 * std::pow(M, 2) * std::pow(Mll, 2) * t + 3 * std::pow(Mll, 4) * t -
         16 * E * M * std::pow(t, 2) + 4 * std::pow(M, 2) * std::pow(t, 2) +
         3 * std::pow(Mll, 2) * std::pow(t, 2) - 3 * std::pow(t, 3) +
         2 * a * b * (4 * E * M - 5 * std::pow(Mll, 2) + 3 * t) +
         std::pow(b, 2) *
             (4 * E * M - 4 * std::pow(M, 2) - 3 * std::pow(Mll, 2) + 3 * t) +
         std::pow(a, 2) * (-8 * std::pow(Mll, 2) + 4 * t))) /
           (std::pow(Mll, 4) * std::pow(4 * E * M - std::pow(Mll, 2) + t, 2)) -
       (std::pow(ReT1, 2) * (4 * std::pow(M, 2) - t) *
        (16 * std::pow(E, 2) * std::pow(M, 2) * std::pow(Mll, 2) -
         8 * E * M * std::pow(Mll, 4) - 28 * std::pow(M, 2) * std::pow(Mll, 4) +
         std::pow(Mll, 6) - 32 * std::pow(E, 2) * std::pow(M, 2) * t +
         ...
         std::pow(a, 2) * (-8 * std::pow(Mll, 2) + 4 * t))) /
           (std::pow(Mll, 4) * std::pow(4 * E * M - std::pow(Mll, 2) + t, 2)) -
       (8 * b * GEt * M * ReT1 *
        (std::pow(b, 2) * std::pow(M, 2) + a * b * t +
         2 * E * M * (std::pow(Mll, 2) - t) * t +
         t * (std::pow(a, 2) - std::pow(Mll, 2) * t) -
         std::pow(M, 2) * (std::pow(Mll, 4) + 4 * std::pow(E, 2) * t -
                           6 * std::pow(Mll, 2) * t + std::pow(t, 2)))) /
           (L * std::pow(Mll, 2) * (-4 * E * M + std::pow(Mll, 2) - t) * t) +
       (std::pow(GEt, 2) * std::pow(M, 2) *
        (std::pow(b, 4) * (2 * std::pow(m, 2) + 4 * E * M + 4 * std::pow(M, 2) -
                           std::pow(Mll, 2)) +
         8 * std::pow(a, 2) * std::pow(m, 2) *
             (std::pow(b, 2) + std::pow(std::pow(Mll, 2) - t, 2)) +
         2 * a *
             (b *
                  (-4 * E * M * (8 * std::pow(m, 2) + std::pow(Mll, 2) - t) +
                   (std::pow(Mll, 2) - t) *
                       (12 * std::pow(m, 2) + std::pow(Mll, 2) - t)) *
                  (std::pow(Mll, 2) - t) +
              std::pow(b, 3) *
                  (4 * std::pow(m, 2) + 4 * E * M - std::pow(Mll, 2) + t)) +
         std::pow(std::pow(Mll, 2) - t, 2) *
             (-((4 * std::pow(M, 2) - std::pow(Mll, 2)) *
                std::pow(std::pow(Mll, 2) - t, 2)) +
              16 * std::pow(E, 2) * std::pow(M, 2) *
                  (2 * std::pow(m, 2) + std::pow(Mll, 2) - t) -
              8 * E * M * (std::pow(Mll, 2) - t) *
                  (2 * std::pow(m, 2) + std::pow(Mll, 2) - t) +
              2 * std::pow(m, 2) *
                  (std::pow(Mll, 4) + 16 * std::pow(M, 2) * t -
                   2 * std::pow(Mll, 2) * t - 3 * std::pow(t, 2))) +
         4 * std::pow(b, 2) *
             (E * M * (std::pow(Mll, 2) - t) *
                  (-12 * std::pow(m, 2) + std::pow(Mll, 2) - t) +
              4 * std::pow(E, 2) * std::pow(M, 2) *
                  (2 * std::pow(m, 2) - std::pow(Mll, 2) + t) +
              std::pow(m, 2) *
                  (3 * std::pow(Mll, 4) - 6 * std::pow(Mll, 2) * t +
                   t * (8 * std::pow(M, 2) + t))))) /
           (std::pow(L, 2) * (4 * std::pow(M, 2) - t) * std::pow(t, 2)) +
       (2 * GEt * M * ReT1 * (std::pow(Mll, 2) - t) *
        (-4 * std::pow(a, 3) * (std::pow(Mll, 2) - t) +
         8 * std::pow(a, 2) * b * (E * M - std::pow(Mll, 2) + t) +
         b * (-32 * std::pow(E, 3) * std::pow(M, 3) +
              std::pow(b, 2) *
                  (2 * E * M + 4 * std::pow(M, 2) - std::pow(Mll, 2)) +
              32 * std::pow(E, 2) * std::pow(M, 2) * (std::pow(Mll, 2) - t) +
              2 * E * M * (std::pow(Mll, 2) - t) *
                  (8 * std::pow(M, 2) - 5 * std::pow(Mll, 2) + 3 * t) +
              (std::pow(Mll, 2) - t) *
                  (4 * std::pow(M, 2) * (-3 * std::pow(Mll, 2) + t) +
                   std::pow(Mll, 2) * (std::pow(Mll, 2) + t))) +
         a * (std::pow(b, 2) * (8 * E * M + 8 * std::pow(M, 2) -
                                5 * std::pow(Mll, 2) + 3 * t) +
              (std::pow(Mll, 2) - t) * (16 * std::pow(E, 2) * std::pow(M, 2) -
                                        16 * std::pow(M, 2) * std::pow(Mll, 2) +
                                        8 * E * M * (-std::pow(Mll, 2) + t) +
                                        std::pow(std::pow(Mll, 2) + t, 2))))) /
           (L * std::pow(Mll, 2) * t *
            std::pow(4 * E * M - std::pow(Mll, 2) + t, 2)) +
       (2 * std::pow(GMt, 2) *
        (a * b * t * (-4 * std::pow(m, 2) - 4 * E * M + std::pow(Mll, 2) + t) +
         std::pow(b, 2) * (-4 * std::pow(m, 2) * std::pow(M, 2) -
                           2 * std::pow(M, 2) * std::pow(Mll, 2) -
                           2 * E * M * t + std::pow(Mll, 2) * t) +
         2 * (8 * std::pow(m, 4) * (4 * std::pow(M, 2) - t) * t +
              2 * E * M * std::pow(Mll, 2) * t * (-std::pow(Mll, 2) + t) +
              std::pow(t, 2) * (std::pow(a, 2) + std::pow(Mll, 2) * t) +
              std::pow(M, 2) * std::pow(Mll, 2) *
                  (std::pow(Mll, 4) + 4 * std::pow(E, 2) * t -
                   2 * std::pow(Mll, 2) * t - 3 * std::pow(t, 2)) +
              2 * std::pow(m, 2) *
                  (2 * E * M * t * (-std::pow(Mll, 2) + t) +
                   t * (-std::pow(a, 2) + std::pow(Mll, 2) * t) +
       ...
       (std::pow(GMt, 2) *
        (a * b *
         ...
         a * std::pow(b, 3) * t *
             (4 * std::pow(m, 2) + 4 * E * M - std::pow(Mll, 2) + t) +
         std::pow(b, 4) * (4 * std::pow(m, 2) * std::pow(M, 2) +
                           2 * std::pow(M, 2) * (std::pow(Mll, 2) - t) +
                           2 * E * M * t + t * (-std::pow(Mll, 2) + t)) +
         std::pow(b, 2) *
             (16 * std::pow(m, 4) * t * (-4 * std::pow(M, 2) + t) +
              (std::pow(Mll, 2) - t) *
                  (2 * E * M * (std::pow(Mll, 2) - t) * t +
                   std::pow(Mll, 2) * (std::pow(Mll, 2) - t) * t -
                   4 * std::pow(M, 2) *
                       (std::pow(Mll, 4) + 2 * std::pow(E, 2) * t -
                        std::pow(Mll, 2) * t)) -
              4 * std::pow(m, 2) *
                  (6 * E * M * (std::pow(Mll, 2) - t) * t +
                   2 * std::pow(M, 2) *
                       (std::pow(Mll, 4) - 2 * std::pow(E, 2) * t +
                        std::pow(t, 2)) -
                   t * (std::pow(a, 2) + 2 * std::pow(Mll, 4) -
                        3 * std::pow(Mll, 2) * t + 2 * std::pow(t, 2)))) +
         std::pow(std::pow(Mll, 2) - t, 2) *
             (16 * std::pow(m, 4) * t * (-4 * std::pow(M, 2) + t) +
              (std::pow(Mll, 2) - t) *
                  (4 * E * M * t * (-std::pow(Mll, 2) + t) +
                   std::pow(t, 2) * (-std::pow(Mll, 2) + t) +
                   2 * std::pow(M, 2) *
                       (std::pow(Mll, 4) + 4 * std::pow(E, 2) * t -
                        std::pow(t, 2))) +
              4 * std::pow(m, 2) *
                  (2 * E * M * t * (-std::pow(Mll, 2) + t) +
                   std::pow(M, 2) *
                       (std::pow(Mll, 4) + 4 * std::pow(E, 2) * t +
                        2 * std::pow(Mll, 2) * t - 7 * std::pow(t, 2)) +
                   t * (std::pow(a, 2) - std::pow(Mll, 2) * t +
                        2 * std::pow(t, 2)))))) /

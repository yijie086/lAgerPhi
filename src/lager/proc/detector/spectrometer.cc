...
// TODO this is the same in cone.cc, factorize
namespace {
...
spectrometer::spectrometer(const configuration& cf, const string_path& path,
  ...
  LOG_INFO(name_,
           "Rotation [deg.]: " +
               std::to_string(cf.get<double>(path / "position" / "phi0")));
  LOG_INFO(name_, "Acceptance: " + std::to_string(acceptance_));
  LOG_INFO(name_, "Central momentum [GeV]: " + std::to_string(p0_));
  LOG_INFO(name_,
           "Inbending angle range [rad]: [" + std::to_string(th_in_.min) +
               ", " + std::to_string(th_in_.max) + "]");
  LOG_INFO(name_,
           "Outbending angle range [rad]: [" + std::to_string(th_out_.min) +
  ...
  for (auto& part : e) {
    if (part.final_state() &&
      ...
      if (p_.includes(part.momentum())) {
        ...
        LOG_JUNK2(name_,
                  "True th_in: " + std::to_string(th_in) +

#ifndef LIEGE_CORE_LOGGER_LOADED
#define LIEGE_CORE_LOGGER_LOADED
...
#define LOG_CRITICAL(mtitle, mtext)                                            \
  if (liege::global::logger.level() >= log_level::CRITICAL) {               \
    liege::log<liege::log_level::CRITICAL>((mtitle), (mtext));           \
  }
#define LOG_ERROR(mtitle, mtext)                                               \
  if (liege::global::logger.level() >= log_level::ERROR) {                  \
    liege::log<liege::log_level::ERROR>((mtitle), (mtext));              \
  }
#define LOG_WARNING(mtitle, mtext)                                             \
  if (liege::global::logger.level() >= log_level::WARNING) {                \
    liege::log<liege::log_level::WARNING>((mtitle), (mtext));            \
  }
#define LOG_INFO(mtitle, mtext)                                                \
  if (liege::global::logger.level() >= log_level::INFO) {                   \
    liege::log<liege::log_level::INFO>((mtitle), (mtext));               \
  }
#define LOG_DEBUG(mtitle, mtext)                                               \
  if (liege::global::logger.level() >= log_level::DEBUG) {                  \
    liege::log<liege::log_level::DEBUG>((mtitle), (mtext));              \
  }
#define LOG_JUNK(mtitle, mtext)                                                \
  if (liege::global::logger.level() >= log_level::JUNK) {                   \
    liege::log<liege::log_level::JUNK>((mtitle), (mtext));               \
  }
#define LOG_JUNK2(mtitle, mtext)                                               \
  if (liege::global::logger.level() >= log_level::JUNK2) {                  \
    liege::log<liege::log_level::JUNK2>((mtitle), (mtext));              \
  }

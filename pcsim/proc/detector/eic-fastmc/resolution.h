...
class resolution{
 public:
   resolution()
       : resolution("det1", "v1", "ebeam_5GeV", "ibeam_100GeV", "solenoidon") {}
   resolution(const char detector[], const char version[], const char ebeam[],
...
//   const int mm=9;
//   char * title[9] = {"acc_eledipole1side","acc_eledipole3back","acc_barrel","acc_ionside","acc_eleside_outer","acc_eleside_inner","acc_iondownstream","acc_iondipole2back","acc_iondipole3back"};    

  
};
#ifdef JLEIC_IMPL

resolution::resolution(const char detector[],const char version[],const char ebeam[],const char ibeam[],const char solenoid[],bool debug){
    if(debug) std::cout <<"construct resolution class" << std::endl;          
  _status = 0;
  bool control = true;
  //file names
  ...
  if (strcmp(detector, "det1") == 0 && strcmp(version, "v1") == 0){
    _status = 1;    
    if(debug)  std::cout << detector << " " << version << " " << ebeam << " " << ibeam << " " << solenoid << std::endl;    
    if(strcmp(ebeam, "ebeam_5GeV") == 0 && strcmp(ibeam, "ibeam_100GeV") == 0){
...
int resolution::get_resolution(const int pid,const double * kin,const int region,double *res,double *kin_smeared,bool debug)
{ 
...
//   }
  ...
  if (kin[1] > 180.0 || kin[1] < 0.0){
    std::cout << "Error in resolution::get_resolution!" << std::endl;
    std::cout << "  Theta angle " << kin[1] << " out of range [0, 180]." << std::endl;
    return 1;
  } 
  if (kin[1] > 180.0 || kin[1] < -180.0){
    std::cout << "Error in resolution::get_resolution!" << std::endl;
    std::cout << "  Phi angle " << kin[2] << " out of range [0, 180]." << std::endl;
    return 1;
  } 
  ...
  kin_smeared[0]=0;  kin_smeared[1]=0; kin_smeared[2]=0;      
...
// delta_p/p(%), theta(deg), phi(deg), vertexz(cm)
  ...
  res[3]=res_region[region][3];
    ...
    if(debug)  cout << res[0] << " " << res[1] << " " << res[2] << " " << res[3] << endl;        
    ...
    double res_factor=1;      
    TRandom3 rnd;      
    kin_smeared[0]=rnd.Gaus(kin[0],res_factor*res[0]/100.*kin[0]);
    kin_smeared[1]=rnd.Gaus(kin[1],res_factor*res[1]);
    kin_smeared[2]=rnd.Gaus(kin[2],res_factor*res[2]);
    kin_smeared[3]=sqrt(kin_smeared[0]*kin_smeared[0]+kin[3]*kin[3]-kin[0]*kin[0]);
    if(debug){
      cout << kin_smeared[0] << " " << kin_smeared[1] << " " << kin_smeared[2] << " " << kin_smeared[3] << endl;    
      cout << "end smear" << endl;  
    }

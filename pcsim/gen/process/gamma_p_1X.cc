...
factory<gamma_p_1X> gamma_p_1X::factory;
...
gamma_p_1X_data(const beam::photon_data& photon, const beam::data& target,
                const particle& X1, const double xs, const double R)
    : generator_data{xs}
    , R_{R}
...
gamma_p_1X_data gamma_p_1X_qpq::generate(const beam::photon_data photon,
  ...
  const double xs_photo = sigma(W2);
  ...
  return {photon, target, qpq_vm_, recoil_, xs, xs_R};
}
...
// =============================================================================
gamma_p_1X_qpq::calc_max_xsec(const configuration& cf) const {
  ...
  // get the minimum W
  const double W2min =
      opt_range ? fmin(opt_range_->min * opt_range_->min, target.mass2())
                : target.mass2();
...
// =============================================================================
gamma_p_1X_qpq::sigma(const double W2) const {
  return qpq_coupling_ * qpq_coupling_ * qpq_amplitude *
         TMath::Gaus(sqrt(W2), qpq_.mass(), qpq_.width());
}
gamma_p_1X_qpq::R(const double Q2) const {
  return physics::R_vm_martynov(Q2, vm_pole_.mass(), R_vm_c_, R_vm_n_);
}
gamma_p_1X_qpq::dipole(const double Q2) const {
